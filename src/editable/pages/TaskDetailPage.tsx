...
function BackLink({ task }: { task: TaskKey }) {
  ...
  return (
    <Link href={taskConfig?.route || '/'} className="inline-flex items-center gap-2 rounded-full border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)]/70 px-4 py-2 text-sm font-extrabold">
...
function ArticleDetail({ post, related, comments }: { post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_350px] lg:px-8 lg:py-16">
      <article className="min-w-0 rounded-[2.7rem] border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] p-5 shadow-[0_30px_90px_rgba(15,23,42,0.09)] sm:p-8 lg:p-12">
        <BackLink task="article" />
        <p className="mt-8 text-xs font-extrabold uppercase tracking-[0.28em] text-[var(--slot4-accent)]">{categoryOf(post, 'Article')}</p>
        <h1 className="mt-4 text-4xl font-extrabold leading-[0.98] tracking-[-0.07em] sm:text-5xl lg:text-7xl">{post.title}</h1>
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
        <article className="rounded-[2.8rem] border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] p-6 shadow-[0_30px_90px_rgba(15,23,42,0.09)] sm:p-9">
          <div className="grid gap-6 sm:grid-cols-[150px_1fr]">
            <div className="flex h-36 w-36 items-center justify-center overflow-hidden rounded-[2rem] bg-[var(--slot4-page-bg)] ring-1 ring-[var(--slot4-line)]">
...
function ClassifiedDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  const website = getField(post, ['website', 'url'])
  return (
    <section className="mx-auto grid max-w-7xl gap-7 px-4 py-10 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:px-8 lg:py-16">
      <aside className="rounded-[2.5rem] border border-[var(--slot4-line)] bg-[var(--slot4-page-text)] p-7 text-[var(--slot4-page-bg)] shadow-xl lg:sticky lg:top-24 lg:self-start">
      ...
      <article className="rounded-[2.7rem] border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] sm:p-9">
  ...
  return (
    <>
      <section className="mk-dark-pattern relative overflow-hidden px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
        <span className="mk-drift pointer-events-none absolute left-[7%] top-[42%] h-10 w-10 rounded-full border border-[var(--slot4-accent)]" />
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.78fr_1.22fr] lg:items-center">
          <aside>
            <BackLink task="image" />
            <div className="mt-8 inline-flex items-center gap-2 rounded-full bg-[var(--slot4-accent)] px-4 py-2 text-xs font-extrabold uppercase tracking-[0.2em] text-white"><Camera className="h-4 w-4" /> Image story</div>
            <h1 className="mt-6 text-4xl font-extrabold leading-[1.03] tracking-[-0.05em] text-white sm:text-6xl">{post.title}</h1>
          </aside>
          <div className="mk-browser-card mk-float bg-[var(--slot4-dark-bg)] p-3 pt-11 shadow-[0_34px_80px_rgba(0,0,0,0.32)]">
            <div className="relative aspect-[16/10] overflow-hidden rounded-md bg-[var(--slot4-surface-bg)]">
              <img src={images[0] || '/placeholder.svg?height=900&width=1200'} alt="" className="absolute inset-0 h-full w-full object-cover" />
            </div>
          </div>
        </div>
      </section>
      <section className="mx-auto grid max-w-7xl gap-8 px-4 py-16 sm:px-6 lg:grid-cols-[0.72fr_1.28fr] lg:px-8">
        <aside className="rounded-[1.5rem] border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] p-7 shadow-[0_20px_50px_rgba(23,23,25,0.06)] lg:sticky lg:top-24 lg:self-start">
          <p className="text-xs font-extrabold uppercase tracking-[0.22em] text-[var(--slot4-accent)]">Project notes</p>
        ...
        <div className="grid gap-7 md:grid-cols-2">
          {(images.length ? images : ['/placeholder.svg?height=900&width=1200']).map((image, index) => (
            <figure key={`${image}-${index}`} className="mk-browser-card overflow-hidden">
              ...
              {index === 0 ? <figcaption className="pt-5 text-sm font-bold text-[var(--slot4-muted-text)]">Featured visual from this image post.</figcaption> : null}
            </figure>
          ))}
        </div>
      </section>
      <section className="mx-auto max-w-7xl px-4 pb-16 sm:px-6 lg:px-8"><RelatedPanel task="image" post={post} related={related} /></section>
    </>
  )
}
...
function BookmarkDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="rounded-[2.7rem] border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] p-7 shadow-[0_30px_90px_rgba(15,23,42,0.08)] sm:p-10">
        <BackLink task="sbm" />
        <div className="mt-10 flex h-20 w-20 items-center justify-center rounded-[2rem] bg-[var(--slot4-page-text)] text-[var(--slot4-page-bg)]"><Bookmark className="h-9 w-9" /></div>
    ...
    <section className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="rounded-[2.7rem] border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] sm:p-9">
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-[120px_1fr]">
          <div className="flex h-28 w-28 items-center justify-center rounded-[1.8rem] bg-[var(--slot4-page-text)] text-[var(--slot4-page-bg)]"><FileText className="h-12 w-12" /></div>
          <div>
        ...
        <BodyContent post={post} />
        {fileUrl ? (
          <div className="mt-8 overflow-hidden rounded-[2rem] border border-[var(--slot4-line)] bg-[var(--slot4-page-bg)]">
            <div className="flex items-center justify-between gap-3 border-b border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] p-4">
...
function ProfileDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  const email = getField(post, ['email'])
  return (
    <section className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[420px_minmax(0,1fr)] lg:px-8 lg:py-16">
      <aside className="rounded-[2.7rem] border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] p-8 text-center shadow-[0_30px_90px_rgba(15,23,42,0.08)] lg:sticky lg:top-24 lg:self-start">
        <BackLink task="profile" />
        <div className="mx-auto mt-10 flex h-40 w-40 items-center justify-center overflow-hidden rounded-full bg-[var(--slot4-page-bg)] ring-1 ring-[var(--slot4-line)]">
      ...
      <article className="rounded-[2.7rem] border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] p-7 shadow-sm sm:p-10">
        <BodyContent post={post} />
  ...
  return (
    <div className="mt-8 grid gap-3 sm:grid-cols-2">
      {visible.map(([label, value, Icon]) => (
        <div key={label} className="rounded-[1.5rem] border border-[var(--slot4-line)] bg-[var(--slot4-page-bg)] p-4">
...
function MapBox({ src, label }: { src: string; label: string }) {
  return (
    <div className="overflow-hidden rounded-[2rem] border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] shadow-sm">
...
function ContactAction({ website, phone, email }: { website?: string; phone?: string; email?: string }) {
  ...
  return (
    <div className="mt-5 rounded-[2rem] border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] p-5 shadow-sm">
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        ...
        {phone ? <a href={`tel:${phone}`} className="inline-flex items-center gap-2 rounded-full border border-[var(--slot4-line)] px-4 py-2 text-sm font-extrabold"><Phone className="h-4 w-4" /> Call</a> : null}
        {email ? <a href={`mailto:${email}`} className="inline-flex items-center gap-2 rounded-full border border-[var(--slot4-line)] px-4 py-2 text-sm font-extrabold"><Mail className="h-4 w-4" /> Email</a> : null}
      </div>
    </div>
  )
}
...
function RelatedPanel({ task, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-[2rem] border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)]/70 p-5 backdrop-blur">
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 rounded-2xl border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] p-3 transition hover:-translate-y-0.5 hover:shadow-lg">
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-10 rounded-[2rem] border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)]/70 p-5">
      <div className="flex items-center gap-2 text-lg font-extrabold"><MessageCircle className="h-5 w-5" /> Comments</div>
      <div className="mt-5 grid gap-3">
        {comments.slice(0, 5).map((comment) => (
          <div key={comment.id} className="rounded-2xl border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] p-4">
