    ...
    <EditableSiteShell>
      <main className="bg-[var(--slot4-page-bg)] px-4 py-14 text-[var(--slot4-page-text)] sm:px-6 lg:px-8">
        <section className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <article className="rounded-[2.5rem] border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)]/80 p-8 shadow-sm lg:p-12">
            <p className="text-xs font-extrabold uppercase tracking-[0.24em] opacity-55">{pagesContent.about.badge}</p>
            <h1 className="mt-5 text-5xl font-extrabold tracking-[-0.07em]">About {SITE_CONFIG.name}</h1>
          ...
          <aside className="space-y-4">
            {pagesContent.about.values.map((value) => (
              <div key={value.title} className="rounded-[2rem] border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)]/70 p-6 shadow-sm">
