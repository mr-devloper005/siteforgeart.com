      ...
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        <section className="rounded-[2rem] border border-border bg-card p-6 shadow-sm sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <button type="button" className="rounded-full border border-[var(--slot4-line)] px-4 py-2 text-sm font-extrabold" onClick={refreshComments}>Refresh comments</button>
          </div>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-md">
              ...
              <input
                ...
                className="h-11 w-full rounded-2xl border border-[var(--slot4-line)] bg-white pl-9 pr-3 text-sm outline-none"
              />
            </div>
        ...
        {filtered.length > COMMENTS_PER_PAGE ? (
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-card p-4 text-sm text-muted-foreground">
            ...
            <div className="flex gap-2">
              <button type="button" className="rounded-full border border-[var(--slot4-line)] px-4 py-2 font-extrabold disabled:opacity-40" disabled={currentPage <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>Previous</button>
              <button type="button" className="rounded-full border border-[var(--slot4-line)] px-4 py-2 font-extrabold disabled:opacity-40" disabled={currentPage >= totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>Next</button>
            </div>
          </div>
        ) : null}
      </main>
